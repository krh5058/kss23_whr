...
% Set-up left pane
left1 = JPanel(GridLayout(3,1));
left1.setMinimumSize(Dimension(150,325));
left1.setPreferredSize(Dimension(150,325));
left1.add(tf1Panel);
left1.add(tf2Panel);
left1.add(rb2Panel);
...
headArray(1) = java.lang.String('Block Order');
...
right1.setMinimumSize(Dimension(250,325));
right1.setPreferredSize(Dimension(250,325));
...
componentsCell = cell([4 4]); % 4 components, 4 sections
...
callback4 = @(obj,evt)onPlay(obj,evt);
set(pbh,'MouseClickedCallback', callback4);

% Set-up right pane
right2 = JPanel(GridLayout(2,1));
...
frame.setVisible(1);
    ...
    function onPlay(obj,evt)
        ...
        picture.setIcon(javax.swing.ImageIcon.createImageIcon('content/Stim_Bodies/WHR_0.66.jpg'));
    end
    ...
    function onConfirm(obj,evt) % When confirm button is pressed
        ...
        elseif any(cellfun(@isempty,listout)) % Check for empty entries in order list
            ...
            if result==javax.swing.JOptionPane.YES_OPTION
                setappdata(frame,'UserData',{char(sid),char(age),char(gender),listout});
                frame.dispose();
            else
            end
        end
